...
class PuzzlebotPublisher(Node):
    ...
    def cmd_cb(self, msg):
        ...
        wr_ideal = (msg.linear.x + (msg.angular.z * L / 2.0)) / r
        wl_ideal = (msg.linear.x - (msg.angular.z * L / 2.0)) / r
        
        if "group1" in self.namespace:
            # El líder es perfecto: no hay ruido entre el actuador y el sensor.
            self.wr = wr_ideal
            self.wl = wl_ideal
            ...
            # Simulación de Error en Actuadores/Sensores:
            # Agregamos ruido para simular que el encoder o el motor no son perfectos.
            std_dev_sensor = 0.01 
            wr_sensor = wr_ideal + random.gauss(0, std_dev_sensor)
            wl_sensor = wl_ideal + random.gauss(0, std_dev_sensor)
            
            self.pub_wr.publish(Float32(data=float(wr_sensor)))
            self.pub_wl.publish(Float32(data=float(wl_sensor)))
    ...
    def timer_cb(self):
        ...
        self.odom_transform.child_frame_id = f"{self.namespace}/odom"
